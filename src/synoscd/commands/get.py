...
def apps(
    ...
    try:
        ...
        for resource in desired.values():
            ...
            desired_suspended = (
                resource.spec.suspend if hasattr(resource.spec, "suspend") else False
            )
            ...
            if live:
                provisioning = live.get("properties", {}).get(
                    "provisioningState", "Unknown"
                )
                ready_rev = live.get("properties", {}).get(
                    "latestReadyRevisionName", "-"
                )
                status_icon = (
                    "✓" if provisioning == "Succeeded" and ready_rev != "-" else "✗"
                )
            else:
...
def get_status(
    ...
    try:
        ...
        managed_desired = [
            ...
            and app.metadata.labels.get("synoscd.io/managed") == "true"
        ]
